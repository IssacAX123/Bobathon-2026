...
class GitHubCommentPoster:
    ...
    def post_analysis(
        ...
        try:
            ...
            # DRY RUN: Just print the comment
            if self.dry_run:
                print("\n" + "="*80)
                ...
                print(comment_body)
                print("\n" + "-"*80)
                print(f"Would add label: bot-analyzed")
                print("="*80 + "\n")
    ...
    def _generate_package_details(self, pkg_name: str, issue_text: str) -> List[str]:
        ...
        if 'crypto.ltpakeyutil' in pkg_name:
            ...
            lines.append("- Private key and shared secret encrypted with password-derived key")
            lines.append("- Password hashed with SHA-256 (FIPS) or SHA-1 (non-FIPS)")
            lines.append("- File permissions should be 600 (owner read/write only)")
            lines.append("- Keys must be identical across all SSO servers\n")
